  ...
  return (
    <section id='works' className='relative h-max w-full pt-[20%] md:pt-0'>
      ...
      <div className='flex h-full w-full flex-col'>
        ...
        <div className='flex basis-[85%] items-center justify-center p-[10%] pl-[15%] md:py-[5%] md:pl-[20%]'>
...
const Card: React.FC<cardProps> = (props) => {
  ...
  return (
    <motion.div
      className='row-span-1 grid w-full gap-8 md:grid-cols-2'
      variants={container}
      initial='hidden'
      whileInView='show'
    ...
    >
      {props?.projects.map((project) => (
        <motion.div
          variants={item}
          className='group relative flex min-h-[300px] flex-col justify-between overflow-hidden rounded-2xl border border-[rgba(255,255,255,0.08)] bg-[#141414] p-8 transition-all duration-500 hover:border-[rgba(255,255,255,0.2)] hover:bg-[#1a1a1a]'
          key={project?.id}
        >
          {/* Decorative Elements */}
          <div className='pointer-events-none absolute top-0 right-0 h-[200px] w-[200px] bg-[radial-gradient(circle_at_center,_var(--purple)_0%,_transparent_70%)] opacity-0 blur-[60px] transition-opacity duration-700 ease-in-out group-hover:opacity-15' />

          <div className='relative z-10'>
            <div className='mb-2 text-xs font-bold tracking-widest text-[var(--golden)] uppercase opacity-80'>
              {project?.context}
            </div>
            <div className='mb-4 text-2xl font-bold tracking-tight text-[var(--secondary)] transition-all duration-300 group-hover:bg-gradient-to-r group-hover:from-white group-hover:to-purple-300 group-hover:bg-clip-text group-hover:text-transparent'>
              {project?.title}
            </div>
            <div className='mb-6 text-sm leading-relaxed font-light text-[var(--secondary-grey)]'>
              {project?.description}
            </div>
            {project?.techStack && (
              <div className='mb-4 flex flex-wrap gap-2'>
                {project.techStack.split(',').map((tech, i) => (
                  <span
                    key={i}
                    className='rounded border border-[rgba(255,255,255,0.05)] bg-[rgba(255,255,255,0.05)] px-2 py-1 text-[10px] tracking-wider text-[var(--text-grey)] uppercase'
                  >
                    {tech.trim()}
                  </span>
                ))}
